...
def index():
    ...
    # Create the link texts
    about_link = f'<a href="{about_url}">About</a>'
    game_link = f'<a href="{game_url}">Game</a>'

    # Combine the links in a string
    links_text = f'The URLs for this page are: {about_link}, {game_link}'

    return make_response(render_template("lab_javascript_index.html", links_text=links_text))
...
@app.route('/about')
def about():
    return make_response(render_template("lab_javascript_about.html"))
